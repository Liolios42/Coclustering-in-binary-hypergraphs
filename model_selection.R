...
n.cores <- 32
em.runs <- 3
N <- c(500)# Number of nodes
M <- c(500) # Number of hyperedges
K <- c(2, 3, 4) # Number of node-groups
G <- c(2, 3, 4)  # Number of hyperedge-groups
n = 500
m = 500
...
gamma <- c(0.6, 0.3, 0.1)
delta <- c(0.7, 0.2, 0.1)
p_kg <- c(0.9, 0.6, 0.4, 0.2, 0.8, 0.4, 0.2, 0.1, 0.7)
    ...
    for (k in K) {
      for (g in G) {
        ...
        for(replic in seq(mc.replicates)){
          ...
          synthetic_data <- data_generation(500, 500, 3, 3, gamma, delta, p_kg)
          ...
          for (run in seq(em.runs)) {
            ...
            constants2 <- c(random_hypergraph, 500, 500, k, g)
	    ...
	    hyper.clusters.est <- mapping$hyper_labels[hyper.clusters.est]
            ...
            if(!any(is.na(node.clusters.est)) & !any(is.na(hyper.clusters.est))){
              ICL_41<- BinBlocICL_LBM(a = 4, b = 1, x_ij, hyper.clusters.est, node.clusters.est)
              ICL_11 <- BinBlocICL_LBM(a = 1, b = 1, x_ij, hyper.clusters.est, node.clusters.est)
            ...
            results <- rbind(results, data.frame(K = k, G = g, N = n, M = m, replicate = replic, em_run = run, ICL41 = ICL_41, ICL11 = ICL_11,
            ...
            write.csv(results, file = 'auc_outputs/exploration.csv')
	    saveRDS(em_list, file = 'auc_outputs/exploration.RDS')
...
write.csv(results, file = 'auc_outputs/exploration2.csv')
saveRDS(em_list, file = 'auc_outputs/exploration.RDS')
